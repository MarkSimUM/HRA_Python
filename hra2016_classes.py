...
class SurveyVariableList:
    def __init__(self):
        # This class holds lists of survey / biometric variables used by other classes
        # Primary lists to use:
        #   varlist => self reported survey variables
        #   biolist => biometric data
        # vlist_[sectionname] => these lists are subsets of variables by section prefix
        # lst_prefix => list of prefixes [n=21]
        # mdclist => special list of mdc variables without prefix (e.g, mdc_stroke is "stroke")
        
        # Prefixes used for survey variables
        self.lst_prefix = ['demo','sbio','tob','aod','drive','nutr','physact','sc','ql','wh'
                           ,'mdc','mdctx', 'famhx','ps','sc','plan','info','job'
                           ,'carehrs','wlq','hpq']

        # create list of variables for each prefix
        self.vlist_demo =['demo_gender','demo_pregnant','demo_age'
                          ,'demo_maritalstatus','demo_race','demo_multiracial','demo_education','demo_employstatus','demo_income']


        self.vlist_sbio = ['sbio_height_feet','sbio_height_inches','sbio_weight','sbio_waist_inches'
                           ,'sbio_bpsystolic','sbio_bpdiastolic','sbio_bp_dk','sbio_choltc','sbio_choltc_dk'
                           ,'sbio_cholhdl','sbio_cholhdl_dk'
                           ,'sbio_cholldl','sbio_cholldl_dk'
                           ,'sbio_trig','sbio_trig_dk'
                           ,'sbio_glucose','sbio_glucose_dk','sbio_glucose_testtype'
                           ,'sbio_hba1c','sbio_hba1c_dk']
        self.vlist_tob =  ['tob_cigssmokestatus','tob_cigsperday','tob_cigsyearssmoked','tob_cigsyearsquit','tob_cigsavgbeforequit'
                           ,'tob_othercigars','tob_otherpipes','tob_othersmokeless','tob_otherecigs','tob_othernone']
        self.vlist_aod = ['aod_drugmedfreq','aod_alcoh_drinksperweek','aod_alcoh_drvridedrunkfreq']
        self.vlist_drive = ['drive_cardistance','drive_motorcycledistance','drive_seatbeltuse','drive_speed','drive_usuallytravel']
        self.vlist_nutr = ['nutr_fiberservings','nutr_fatcholservings']
        self.vlist_physact = [ 'physact_vigorousfreq','physact_vigorousminutes','physact_moderatefreq','physact_moderateminutes'
                               ,'physact_strengthfreq','physact_stretchfreq','physact_limitation','physact_sedentarytime']
        self.vlist_sc = ['sc_flossfreq','sc_sunscreenfreq','sc_sleepduration']
        self.vlist_ql = ['ql_overallhealth','ql_illnessdaysall','ql_lifesatisfaction','ql_socialtiestrength','ql_personalloss','ql_tenseanxiousdepressed','ql_stresseffecthealth']
        self.vlist_wh = ['wh_hysterectomy','wh_agefirstperiod','wh_agefirstchildborn']
        
        # mdclist is variable name only without prefix
        self.mdclist = []

        # "Diseases" in mdc section -> this is for convenience to identify diseases in mdc set of items
        self.vlist_mdc_disease = ['mdc_bronchemph','mdc_cancer','mdc_diabetes','mdc_heartdz','mdc_stroke', 'mdc_chrkidneydz'];
        self.vlist_mdctx_disease = []
      
        # auto create the variables for med condition meds/care
        for item in self.vlist_mdc_disease:
            # remove 'mdc_' prefix
            mdc = item[4:] 
            ...
            self.vlist_mdctx_disease.append(var1);
            self.vlist_mdctx_disease.append(var2);
            self.vlist_mdctx_disease.append(var3);

        # "conditions" in mdc section -> for convenience to identify conditions in mdc set of items
        self.vlist_mdc_condition = ['mdc_allergies','mdc_arthritis','mdc_asthma','mdc_backpain','mdc_chrpain'
                                ,'mdc_depression','mdc_prediabetes','mdc_heartburn','mdc_hibp','mdc_hichol'
                                ,'mdc_menopause','mdc_migraine','mdc_osteop','mdc_sleepdisorder','mdc_other']
        self.vlist_mdctx_condition =[]

        # auto create the variables for med condition meds/care
        for item in self.vlist_mdc_condition:
            # remove 'mdc_' prefix
            mdc = item[4:] 
            var1 = 'mdctx_'+mdc+'_meds'
            var2 = 'mdctx_'+mdc+'_care'
            var3 = 'mdctx_'+mdc+'_none'
            self.mdclist.append(mdc);
            self.vlist_mdctx_condition.append(var1);
            self.vlist_mdctx_condition.append(var2);
            self.vlist_mdctx_condition.append(var3);

        # use disease and condition lists to create full list for mdc section
        self.vlist_mdc = [];
        self.vlist_mdc.extend(self.vlist_mdc_disease);
        self.vlist_mdc.extend(self.vlist_mdc_condition);
        
        # use disease and condition lists to create full list for mdctx section
        self.vlist_mdctx = []
        self.vlist_mdctx.extend(self.vlist_mdctx_disease);
        self.vlist_mdctx.extend(self.vlist_mdctx_condition);

        self.vlist_famhx = ['famhx_breastcancer','famhx_cancer_mother','famhx_cancer_father','famhx_cancer_grands','famhx_cancer_sibling','famhx_cancer_dk'
                        ,'famhx_diabetes_mother','famhx_diabetes_father','famhx_diabetes_grands','famhx_diabetes_sibling','famhx_diabetes_dk','famhx_heartprob_mother'
                        ,'famhx_heartprob_father','famhx_heartprob_grands','famhx_heartprob_sibling','famhx_heartprob_dk','famhx_hibp_mother','famhx_hibp_father','famhx_hibp_grands'
                        ,'famhx_hibp_sibling','famhx_hibp_dk','famhx_hichol_mother','famhx_hichol_father','famhx_hichol_grands','famhx_hichol_sibling','famhx_hichol_dk']
        self.vlist_ps = ['ps_flushot','ps_tetanusshot','ps_bp','ps_chol','ps_dentalexam','ps_cervicalca_paponlytime','ps_cervicalca_paphpvtype'
                        ,'ps_cervicalca_paphpvtime','ps_mammogram','ps_colonca_fobt','ps_colonca_flexsig','ps_colonca_colonoscopy']
        
        self.vlist_sc = ['sc_pastyrmedvisits_office','sc_pastyrmedvisits_er','sc_pastyrmedvisits_hosp']
        self.vlist_plan = ['plan_physact','plan_weight','plan_alcohol','plan_tobacco','plan_nutrfatchol','plan_bp','plan_chol','plan_stress']
        self.vlist_info = ['info_enhancehealth','info_followup']
        self.vlist_job = ['job_satisfaction','job_illnessdays','job_healthaffectprod']
        self.vlist_wlq =['wlq_workreqhours','wlq_startontime','wlq_repeatmotions','wlq_useequipment','wlq_concentrate','wlq_helpothers','wlq_doreqwork','wlq_capablework']
        self.vlist_carehrs = ['carehrs_sickchild','carehrs_sickadult','carehrs_sickelder']
        self.vlist_hpq = ['hpq_weekhrs_past','hpq_weekhrs_expect','hpq_missdays_allself','hpq_missdays_allother','hpq_missdays_ptself'
                          ,'hpq_missdays_ptother','hpq_moredays','hpq_monthhrs_past','hpq_perform_most'
                          ,'hpq_perform_selfyear','hpq_perform_selfmonth']
        

        
        # complete variable list for questionnaire -> self.varlist
        self.varlist = [];
        # create self.varlist by adding all the vlist variables
        for item in self.lst_prefix:
            
            v = 'vlist_'+item;
            lst = getattr(self,v);
            self.varlist.extend(lst);

        
        # biometric screening data (add date)
        self.biolist = ['bio_choltc','bio_cholhdl','bio_cholldl','bio_choltc_mmol','bio_cholhdl_mmol','bio_cholldl_mmol','bio_trig','bio_trig_mmol'
                        ,'bio_bpsystolic','bio_bpdiastolic','bio_glucose','bio_glucose_testtype','bio_hba1c','bio_height_feet'
                        ,'bio_height_inches','bio_height_cm','bio_height_total','bio_waist_inches','bio_waist_cm','bio_weight'
                        ,'bio_weight_kg','bio_bodyfat','bio_psa','bio_pulse','bio_chol_dt','bio_bp_dt'
                        ,'bio_bodyfat_dt','bio_glucose_dt','bio_pulse_dt','bio_waist_dt','bio_weight_dt','bio_hba1c_dt','bio_psa_dt'];

        # check variable list
        #print('self.SurveyVariableList list: ',self.varlist);
...
# This class processes key variables from an sd_hra POST
    ...
    def __init__(self, record):
        ...
        # this class is used for variable lists related to user/project variables
        # keyvarlist => list of variables required to post data
        # critical_error =>   if keyvarlist variable does not exist, critical_error is set to non-zero number
...
# This class processes a single HRA survey record from an sd_hra POST
    ...
    def __init__(self, record, varlist, biolist):


        self.varlist = varlist ;  # self survey items
        self.biolist = biolist ;  # screening data
        ...
        self.varexist(self.varlist);  # set variables that do not exist to -7 (should indicate they are not in questionnaire) 
        self.varexist(self.biolist);  # set variables that do not exist to -7 (should indicate they are not in questionnaire)

        # if biocount > 0, bio variables exist in POST
        self.biocount = self.biodatacount(self.biolist);
        print('bio count in data: ', self.biocount); 
...
# x = HRAData2016([{"demo_gender": 1, "demo_age": 22, "Demo_pregnant":""}]);
    ...
    def __init__(self, record):
        ...
        # use SurveyVariableList class to get variable lists
        
        self.svl = SurveyVariableList();
        # varlist => self reported survey variables
        self.varlist = self.svl.varlist ;
        # biolist => biometric variables from screening.  IF no screening data, these are filled with self reported equivalent
        #    biotype variables store if screening or self-reported 1=self, 2= screen 0=no data (e.g., biotype_trig = 1)
        self.biolist = self.svl.biolist ;


        # [NOTE: this method only for python 3.x] super(HRAData2016, self).__init__(records, self.varlist);
        #   use python 2.x compatible method for initializing parent class
        
        # initialize parent class
        HRAData_Base.__init__(self,record,self.varlist,self.biolist)
        ...
        # TODO: merge bio data from table if previously exists
        self.cleanbiodata();
        # all useable data set into bio_<name> variables, biotype_<name> is type (1=self, 2= screen)
        self.setbiotype();
        ...
        for item in self.svl.mdclist:
            self.skipclean_mdc(item);

        employstatus = self.record['demo_employstatus'];
        if employstatus != 1:
            print('skip logic for skipclean_wlq ...');
            self.skipclean_wlq();        
            print('skip logic for skipclean_hpq start...');
            self.skipclean_hpq();
            
        # TODO: add other skip logic
        # TODO: metric conversions
        # initialize special variables that will be used in calc engine
        
        record.ndiseases = -1 ;
        record.nconditions = -1 ;
    ...
    def calcengine(self):
        print('creating hracalc ....');
        hracalc = hra2016_calcengine.HRACalcData(self.record,self.svl);
        print(' hracalc created....');

    def calcmessages(self):
        hramessage = hra2016_messages.ReportMessageVars2016(self.record);
    ...
    def cleanbiodata(self):
        record = self.record;
        record['bio_height_feet'] =  record['bio_height_feet'] if ( between1(record['bio_height_feet'],2,8 ) ) else -6;
        record['bio_height_inches'] =  record['bio_height_inches'] if ( between1(record['bio_height_inches'],0,11 ) ) else -6;
        record['bio_weight'] =  record['bio_weight'] if ( between1(record['bio_weight'],40,800 ) ) else -6;
        record['bio_waist_inches'] =  record['bio_waist_inches'] if ( between1(record['bio_waist_inches'],10,99 ) ) else -6;
        record['bio_bpsystolic'] =  record['bio_bpsystolic'] if ( between1(record['bio_bpsystolic'],50,350 ) ) else -6;
        record['bio_bpdiastolic'] =  record['bio_bpdiastolic'] if ( between1(record['bio_bpdiastolic'],30,250 ) ) else -6;
        record['bio_choltc'] =  record['bio_choltc'] if ( between1(record['bio_choltc'],60,800 ) ) else -6;
        record['bio_cholhdl'] =  record['bio_cholhdl'] if ( between1(record['bio_cholhdl'],10,400 ) ) else -6;
        record['bio_cholldl'] =  record['bio_cholldl'] if ( between1(record['bio_cholldl'],5,800 ) ) else -6;
        record['bio_trig'] =  record['bio_trig'] if ( between1(record['bio_trig'],5,4000 ) ) else -6;
        record['bio_glucose'] =  record['bio_glucose'] if ( between1(record['bio_glucose'],30,700 ) ) else -6;
        record['bio_glucose_testtype'] =  record['bio_glucose_testtype'] if ( between1(record['bio_glucose_testtype'],1,3 ) ) else -6;
        record['bio_hba1c'] =  record['bio_hba1c'] if ( between1(record['bio_hba1c'],3,30 ) ) else -6;
        # additional screening variables =>
    ...
    def setbiotype(self):
        ...
        # initialize biotype variables : 0=no data, 1=self reported, 2=screening
        record.biotype_height_total = 0;
        record.biotype_choltc = 0;
        record.biotype_cholhdl = 0;
        record.biotype_cholldl = 0;
        record.biotype_trig = 0;
        record.biotype_bpsystolic = 0;
        record.biotype_bpdiastolic = 0;
        record.biotype_glucose = 0;
        record.biotype_hba1c = 0;
        record.biotype_waist_inches = 0;
        record.biotype_weight = 0;
        record.biotype_bodyfat = 0;
        record.biotype_psa = 0;
        record.biotype_pulse = 0;

        if record['bio_height_total'] > 0:
            record.biotype_height_total = 2;
        elif record['bio_height_feet'] > 0:
            record.biotype_height_total = 2;
        elif record['sbio_height_feet'] > 0:
            record.biotype_height_total = 1;
            
        if record['bio_choltc'] > 0:
            record.biotype_choltc = 2;
        elif record['sbio_choltc'] > 0:
            record.biotype_choltc = 1;

        if record['bio_cholldl'] > 0:
            record.biotype_cholldl = 2;
        elif record['sbio_cholldl'] > 0:
            record.biotype_cholldl = 1;
        ...
        if record['bio_trig'] > 0:
            record.biotype_trig = 2;
        elif record['sbio_trig'] > 0:
            record.biotype_trig = 1;

        if record['bio_bpsystolic'] > 0:
            record.biotype_bpsystolic = 2;
        elif record['sbio_bpsystolic'] > 0:
            record.biotype_bpsystolic = 1;

        if record['bio_bpdiastolic'] > 0:
            record.biotype_bpdiastolic = 2;
        elif record['sbio_bpdiastolic'] > 0:
            record.biotype_bpdiastolic = 1;
        
        if record['bio_glucose'] > 0:
            record.biotype_glucose = 2;
        elif record['sbio_glucose'] > 0:
            record.biotype_glucose = 1;

        if record['bio_hba1c'] > 0:
            record.biotype_hba1c = 2;
        elif record['sbio_hba1c'] > 0:
            record.biotype_hba1c = 1;

        if record['bio_waist_inches'] > 0:
            record.biotype_waist_inches = 2;
        elif record['sbio_waist_inches'] > 0:
            record.biotype_waist_inches = 1;

        if record['bio_weight'] > 0:
            record.biotype_weight = 2;
        elif record['sbio_weight'] > 0:
            record.biotype_weight = 1;

        if record['bio_bodyfat'] > 0:
            record.biotype_bodyfat = 2;

        if record['bio_psa'] > 0:
            record.biotype_psa = 2;

        if record['bio_pulse'] > 0:
            record.biotype_pulse = 2;

        # set bio data to self reported if biotype = 1       
            
        if record.biotype_height_total == 1:
            record.bio_height_total =  record.sbio_height_feet*(12) +nvl(record.sbio_height_inches,0) ;
            record.bio_height_inches = record.sbio_height_inches
            record.bio_height_feet = record.sbio_height_feet
            
        elif record.biotype_height_total == 2:
            if record.bio_height_feet > 0:
                record.bio_height_total = record.bio_height_feet*(12) +nvl(record.bio_height_inches,0) ;
            
        if record.biotype_choltc == 1:
            record.bio_choltc = record.sbio_choltc;
        if record.biotype_cholhdl == 1:
            record.bio_cholhdl = record.sbio_cholhdl;
            
        if record.biotype_cholldl == 1:
            record.bio_cholldl = record.sbio_cholldl;
        if record.biotype_trig == 1:
            record.bio_trig = record.sbio_trig;
        if record.biotype_bpsystolic == 1:
            record.bio_bpsystolic = record.sbio_bpsystolic;
        if record.biotype_bpdiastolic == 1:
            record.bio_bpdiastolic = record.sbio_bpdiastolic ;
        if record.biotype_glucose == 1:
            record.bio_glucose = record.sbio_glucose;
        if record.biotype_hba1c == 1:
            record.bio_hba1c = record.bio_hba1c;
            
        if record.biotype_waist_inches == 1:
            record.bio_waist_inches = record.sbio_waist_inches;
        if record.biotype_weight == 1:
            record.bio_weight = record.sbio_weight;
    ...
    def recodedata(self):
        record = self.record;
        # Blood pressure
        if (record['sbio_bpsystolic'] > 0 and record['sbio_bpdiastolic'] > 0):
            if record['sbio_bpsystolic'] < record['sbio_bpdiastolic']:
                bpsystolic = record['sbio_bpsystolic'];
                bpdiastolic = record['sbio_bpdiastolic'];
                record['sbio_bpsystolic'] = bpdiastolic;
                record['sbio_bpdiastolic'] = bpsystolic;
